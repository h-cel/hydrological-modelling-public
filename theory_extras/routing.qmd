...
where $I = I(t)$ as the input is time-varying. Note that the relation between storage and discharge is linear. The Muskingum method is typically applied over a channel reach, defined by its length $\Delta x$. Below, an example is worked out based on @2005Brutsaert. However, the example is 'modernised' with following changes:
...
Following @2005Brutsaert, $\Delta x = 23$ km. Example data for $I$ and $O$ is given in `data/brutsaert_reach.csv` with `I` and `O` in m$^3$/s and `t` in h.  
...
def cost_function_muskingum(params, t_array, I_func, O_obs, delta_x):
    ...
    # Equilibrium initial storage: S0 = K*[(1-X)*O0 + X*I0]
    S0 = np.array([
...
- For the physical interpretation of $X$ tol hold (see @2005Brutsaert), $X \in ]0, 0.5]$ is required. 0.25 is hence a reasonable initial guess.
...
test_cost = cost_function_muskingum(params_0, t_array, fn_I, df_brutsaert["O"].values, delta_x)
```
...
optim_res = scipy.optimize.minimize(
    ...
    params_0,
    args=(t_array, fn_I, df_brutsaert["O"].values, delta_x),
    method=method,
    bounds=bounds,
    tol=1e-3,
    options={"disp": True},
...
```{python}
S0_opt = np.array([
    muskingum_storage(df_brutsaert["O"].iloc[0], K_opt, X_opt, fn_I(t_array[0]))
])  # Initial storage
