...
We reuse the input `I(t)` defined as a piecewise constant function so that `I(m \Delta t_{in})` equals the rainfall input between $m \Delta t_{in}$ and $(m+1) \Delta t_{in}$. 

```{python}
def ode_func(t, S, I_func, K):
    return I_func(t) - S / K
```
...
t_span = (0, 60) # hr
delta_t = 1 # hr
explicit_euler_solution = explicit_euler(ode_func, (0, 60), S0, 1, (I_func, K_num))
```
...
def rk_45(ode_func, t_span:tuple, S0:np.array, args):
    ...
    for i in range(len(t_eval) - 1):
        ...
        sol = scipy.integrate.solve_ivp(
            ode_func, t_interval, S_current, method="RK45", args=args, dense_output=True
        )
        S[:, i + 1] = sol.y[:, -1]
        S_current = sol.y[:, -1]
    return S
...
```{python}
rk45_solution = rk_45(ode_func, (0, 60), S0, (I_func, K_num))
```

TO DO: FLATTEN THE SOLUTIONS FOR PLOTTING
