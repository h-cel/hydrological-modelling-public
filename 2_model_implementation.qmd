...
![Conceptual representation of the hydrological model](figures/FLEX_schets.png){#fig-model-concept width=60%}
...
#| tbl-cap: Model parameters together with their units, minimum and maximum value and a set of uncalibrated values
import pandas as pd

df_parameters = pd.read_csv("data/parameters_new.csv")
parameter_labels = {
    "lambda": r"$\lambda$",
    "S_max": r"$S_{\mathrm{max}}$",
    "b": r"$b$",
    "alpha": r"$\alpha$",
    "Q_p_max": r"$Q_{p,\mathrm{max}}$",
    "beta": r"$\beta$",
    "gamma": r"$\gamma$",
    "S2_max": r"$S_{2,\mathrm{max}}$",
    "kappa_2": r"$\kappa_2$",
    "kappa_1": r"$\kappa_1$",
}
unit_labels = {
...
df_parameters = df_parameters.replace({
    "Parameter": parameter_labels,
    "Units": unit_labels,
})
print(df_parameters.to_markdown(index=False))
```
...
with $\mathcal{f}$ a non-linear function, $\mathbf{x} = [S, S_1, S_2]^T$ the state vector, $\mathbf{f} = [P, E_p]^T$ the forcing vector, and $\mathbf{p} = [\lambda, S_{\mathrm{max}}, b, \alpha, Q_{\mathrm{p,max}}, \beta, \gamma, S_{2,\mathrm{max}}, \kappa_2, \kappa_1]^T$ the parameter vector. For this exercise, the model will be implemented in discrete time using a simple forward Euler scheme^[For more background info on numerical methods for ODEs, see your [bachelor course on differential equations](https://studiekiezer.ugent.be/2025/studiefiche/en/I002428)]:
$$
...
- Implement the model in a separate file `src/hydromod/model.py` as a function. You need to be able to reuse the model in the following exercises.
...
- Not a single figure or metric can summarise the model performance. Nevertheless, some metrics and/or figures are (partially) linked to each other.
...
The forward Euler scheme presented above is the simplest available numerical method to solve the ODEs. [`SciPy`](https://docs.scipy.org/doc/scipy/index.html) offers more advanced numerical time integration methods in `scipy.integrate.solve_ivp`. Investigate how the choice of time integration method affects the model outcome.
