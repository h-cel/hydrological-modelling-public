...
def rk_45(ode_func, t_span: tuple, S0: np.array, delta_t_eval: float, args, **kwargs):
    t_eval = np.arange(t_span[0], t_span[1] + delta_t_eval, delta_t_eval)  # Evaluate at integer hours
    S = np.zeros((len(S0), len(t_eval)))
...
where $I = I(t)$ as the input is time-varying. Note that the reservoir is linear in $K$. The Muskingum method is typically applied over a channel reach, defined by its length $\Delta x$. Below, an example is worked out based on @2005Brutsaert. However, the example is 'modernised' by:
...
def calc_wave_celerity(K, delta_x):
    return delta_x / K


def calc_max_delta_t(K, X, delta_x):
    c_m = calc_wave_celerity(K, delta_x)
...
X_test = 0.25  # -
delta_x = 23_000  # m
max_delta_t = calc_max_delta_t(K_test, X_test, delta_x)  # CFL condition
# Equilibrium initial storage: S0 = K*[(1-X)*O0 + X*I0]
S0_test = np.array([
    muskingum_storage(df_brutsaert["O"].iloc[0], K_test, X_test, fn_I(t_array[0]))
])  # Initial storage
S_explicit_euler_test = explicit_euler(
    ode_func_muskingum, (t_array[0], t_array[-1]), S0_test, 3600, (fn_I, K_test, X_test)
)  # Solve ODE
```

```{python}
S_solution_test = rk_45(
    ode_func_muskingum,
    (t_array[0], t_array[-1]),
    S0_test,
    delta_t_eval=3600,
    args=(fn_I, K_test, X_test),
    max_step=max_delta_t,
)  # Solve ODE
```

```{python}
def cost_function_muskingum(params, t_array, I_func, O_obs):
    K_hours, X = params
...
- $X \in ]0, 0.5] (see @2005Brutsaert), so 0.25 is a reasonable initial guess
...
test_cost = cost_function_muskingum(params_0, t_array, fn_I, df_brutsaert["O"].values)
```
...
optim_res = scipy.optimize.minimize(
    ...
    params_0,
    args=(t_array, fn_I, df_brutsaert["O"].values),
    method=method,
    bounds=bounds,
    tol=1e-3,
    options={"disp": True},
